package lista01_java_exerc_14.src;
//João Papo-de-Pescador, homem de bem, comprou um microcomputador para controlar o rendimento diário de seu trabalho. Toda vez que ele traz um peso de peixes maior que o estabelecido pelo regulamento de pesca do
// estado de São Paulo (50 quilos) deve pagar uma multa de R$ 4,00 por quilo excedente. João precisa que você faça um programa que leia a variável peso (peso de peixes) e
// calcule o excesso. Gravar na variável excesso a quantidade de quilos além do limite e na variável multa 
//o valor da multa que João deverá pagar. Imprima os dados do programa com as mensagens adequadas.






import java.util.Scanner;

public class App {
    public static void main(String[] args) throws Exception {
        double peso,excesso,multa;
       Scanner tec = new Scanner(System.in);

        System.out.println("digite o peso dos peixes em KG ");
       peso = tec.nextDouble();
        tec.close();
        if (peso > 50) {
          excesso = peso - 50;
          multa = excesso * 4;
          System.out.println("Peso dos peixes excedeu o limite em "+ excesso + " kg. O valor da multa é R$ "+ multa);
      } else {
          System.out.println("Peso dos peixes dentro do limite estabelecido.");
      }
      
     

        

    }

}

